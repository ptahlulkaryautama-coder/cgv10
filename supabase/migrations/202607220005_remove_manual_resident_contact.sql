-- Allow removing manually stored resident contact data even when no auth account is linked.
...
begin
  ...
  if target_household.head_user_id is null
    and nullif(btrim(coalesce(target_household.primary_contact_name, '')), '') is null
    and nullif(btrim(coalesce(target_household.primary_phone, '')), '') is null
    and target_household.family_count = 0
    and target_household.vehicle_count = 0 then
    raise exception 'Rumah ini tidak punya akun atau kontak warga yang perlu diremove';
  end if;
  ...
  note_entry := concat(
    ...
    ': ',
    clean_reason,
    coalesce(' | Kontak sebelumnya: ' || nullif(target_household.primary_contact_name, ''), '')
  );
  ...
  values (
    ...
    jsonb_build_object(
      ...
      'removed_contact_name', target_household.primary_contact_name,
